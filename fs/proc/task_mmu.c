			 ...
			 unsigned long *data, unsigned long *resident)
{
	...
	*text = (MM_PAGE_ALIGN(mm, mm->end_code) - (mm->start_code & MM_PAGE_MASK(mm)))
								>> MM_PAGE_SHIFT(mm);
...
static void smaps_account(struct mem_size_stats *mss, struct mm_struct *mm,
		...
		bool locked, bool present)
{
	struct folio *folio = page_folio(page);
	int i, nr = compound ? compound_nr(page) : 1;
	unsigned long size = compound ? folio_size(folio) : MM_PAGE_SIZE(mm);
	unsigned long step_size = compound ? PAGE_SIZE : MM_PAGE_SIZE(mm);
	bool exclusive;
	int mapcount;
		...
		struct mm_walk *walk)
{
	...
	smaps_account(mss, vma->vm_mm, page, false, young, dirty, locked, present);
}
		...
		struct mm_walk *walk)
{
	...
	if (folio_test_anon(folio))
		...
		mss->shmem_thp += folio_size(folio);
	else if (folio_is_zone_device(folio))
		/* pass */;
	else
		mss->file_thp += folio_size(folio);

	smaps_account(mss, vma->vm_mm, page, true, pmd_young(*pmd), pmd_dirty(*pmd),
		      locked, present);
}
#else
static void smaps_pmd_entry(pmd_t *pmd, unsigned long addr,
...
{
	unsigned int idx = (addr & ~MM_PMD_MASK(vma->vm_mm)) >> MM_PAGE_SHIFT(vma->vm_mm);
	u64 flags = 0, frame = 0;
	pmd_t pmd = *pmdp;
	struct page *page = NULL;
	struct folio *folio = NULL;
				  ...
				  unsigned long end, struct mm_walk *walk)
{
	...
	if ((p->arg.flags & PM_SCAN_WP_MATCHING) && !p->vec_out) {
		...
		for (addr = start; addr != end; pte++, addr += MM_PAGE_SIZE(vma->vm_mm)) {
			pte_t ptent = ptep_get(pte);
			...
			if (!flush_end)
				start = addr;
			flush_end = addr + MM_PAGE_SIZE(vma->vm_mm);
		}
	...
	if (!p->arg.category_anyof_mask && !p->arg.category_inverted &&
		...
		for (addr = start; addr < end; pte++, addr += MM_PAGE_SIZE(vma->vm_mm)) {
			unsigned long next = addr + MM_PAGE_SIZE(vma->vm_mm);
			pte_t ptent = ptep_get(pte);
		...
		unsigned long end, struct mm_walk *walk)
{
	...
	} while (pte++, addr += MM_PAGE_SIZE(walk->mm), addr != end);
	pte_unmap_unlock(orig_pte, ptl);
	cond_resched();
	return 0;
}
