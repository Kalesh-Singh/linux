 ...
 */
static int padzero(struct mm_struct *mm, unsigned long address)
{
	unsigned long nbyte;

	nbyte = ELF_PAGEOFFSET(mm, address);
	if (nbyte) {
		nbyte = ELF_MIN_ALIGN(mm) - nbyte;
 ...
 */
static unsigned long elf_map(struct mm_struct *mm, struct file *filep, unsigned long addr,
		const struct elf_phdr *eppnt, int prot, int type,
		unsigned long total_size)
{
	unsigned long map_addr;
	unsigned long size = eppnt->p_filesz + ELF_PAGEOFFSET(mm, eppnt->p_vaddr);
	unsigned long off = eppnt->p_offset - ELF_PAGEOFFSET(mm, eppnt->p_vaddr);
	addr = ELF_PAGESTART(mm, addr);
	size = ELF_PAGEALIGN(mm, size);
	...
	*/
	if (total_size) {
		total_size = ELF_PAGEALIGN(mm, total_size);
 ...
 */
static unsigned long elf_load(struct mm_struct *mm, struct file *filep, unsigned long addr,
		const struct elf_phdr *eppnt, int prot, int type,
		unsigned long total_size)
{
	...
	if (eppnt->p_filesz) {
		map_addr = elf_map(mm, filep, addr, eppnt, prot, type, total_size);
		if (BAD_ADDR(map_addr))
			return map_addr;
		if (eppnt->p_memsz > eppnt->p_filesz) {
			zero_start = map_addr + ELF_PAGEOFFSET(mm, eppnt->p_vaddr) +
				eppnt->p_filesz;
			zero_end = map_addr + ELF_PAGEOFFSET(mm, eppnt->p_vaddr) +
				eppnt->p_memsz;
			 ...
			 */
			if (padzero(mm, zero_start) && (prot & PROT_WRITE))
				return -EFAULT;
		}
	} else {
		map_addr = zero_start = ELF_PAGESTART(mm, addr);
		zero_end = zero_start + ELF_PAGEOFFSET(mm, eppnt->p_vaddr) +
			eppnt->p_memsz;
	}
	if (eppnt->p_memsz > eppnt->p_filesz) {
		/*
		...
		zero_start = ELF_PAGEALIGN(mm, zero_start);
		zero_end = ELF_PAGEALIGN(mm, zero_end);
...
static unsigned long total_mapping_size(struct mm_struct *mm, const struct elf_phdr *phdr, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		if (phdr[i].p_type == PT_LOAD) {
			min_addr = min(min_addr, ELF_PAGESTART(mm, phdr[i].p_vaddr));
...
static unsigned long maximum_alignment(struct mm_struct *mm, struct elf_phdr *cmds, int nr)
{
	...
	return ELF_PAGEALIGN(mm, alignment);
}

/**
 * load_elf_phdrs() - load ELF program headers
 * @mm:       mm_struct for alignment calculations
 ...
 */
static struct elf_phdr *load_elf_phdrs(struct mm_struct *mm, const struct elfhdr *elf_ex,
...
static unsigned long load_elf_interp(struct mm_struct *mm,
		...
		struct arch_elf_state *arch_state)
{
	...
	total_size = total_mapping_size(mm, interp_elf_phdata,
	...
	for (i = 0; i < interp_elf_ex->e_phnum; i++, eppnt++) {
		if (eppnt->p_type == PT_LOAD) {
			...
			map_addr = elf_load(mm, interpreter, load_addr + vaddr,
			...
			if (!load_addr_set &&
			    interp_elf_ex->e_type == ET_DYN) {
				load_addr = map_addr - ELF_PAGESTART(mm, vaddr);
				load_addr_set = 1;
			}
...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	elf_phdata = load_elf_phdrs(bprm->mm, elf_ex, bprm->file);
	if (!elf_phdata)
		goto out;
...
out_free_interp:
	...
	if (interpreter) {
		...
		interp_elf_phdata = load_elf_phdrs(bprm->mm, interp_elf_ex,
						   interpreter);
	...
	for(i = 0, elf_ppnt = elf_phdata;
		...
		if (!first_pt_load) {
			 ...
			 *
			 */
			total_size = total_mapping_size(mm, elf_phdata,
							elf_ex->e_phnum);
			if (!total_size) {
				retval = -EINVAL;
				goto out_free_dentry;
			...
			alignment = maximum_alignment(mm, elf_phdata, elf_ex->e_phnum);
			...
			if (interpreter) {
				 ...
				 */
				if (alignment > ELF_MIN_ALIGN(mm)) {
					load_bias = elf_load(mm, bprm->file, 0, elf_ppnt,
			 ...
			 */
			load_bias = ELF_PAGESTART(mm, load_bias - vaddr);
		}

		error = elf_load(mm, bprm->file, load_bias + vaddr, elf_ppnt,
				elf_prot, elf_flags, total_size);
		if (BAD_ADDR(error)) {
		...
		if (first_pt_load) {
			...
			if (elf_ex->e_type == ET_DYN) {
				load_bias += error -
				             ELF_PAGESTART(mm, load_bias + vaddr);
	...
	if (interpreter) {
		elf_entry = load_elf_interp(mm, interp_elf_ex,
					    interpreter,
	...
	mm->start_brk = mm->brk = ELF_PAGEALIGN(mm, elf_brk);
	...
	if ((current->flags & PF_RANDOMIZE) && snapshot_randomize_va_space > 1) {
		 ...
		 */
		if (!brk_moved)
			mm->brk = mm->start_brk = mm->brk + mm_pte_size(mm);
	...
	if (current->personality & MMAP_PAGE_ZERO) {
		...
		error = vm_mmap(NULL, 0, mm_pte_size(mm), PROT_READ | PROT_EXEC,
				MAP_FIXED | MAP_PRIVATE, 0);

		retval = do_mseal(0, mm_pte_size(mm), 0);
		if (retval)
...
static int fill_files_note(struct memelfnote *note, struct coredump_params *cprm)
{
 ...
 alloc:
	...
	data[0] = count;
	data[1] = ELF_MIN_ALIGN(current->mm);
	/*
...
static int elf_core_dump(struct coredump_params *cprm)
{
	...
	dataoff = offset = roundup(offset, ELF_MIN_ALIGN(current->mm));
