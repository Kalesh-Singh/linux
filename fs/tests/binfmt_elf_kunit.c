...
static void total_mapping_size_test(struct kunit *test)
{
	...
	/* No headers, no size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, NULL, 0), 0);
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, empty, 0), 0);
	/* Empty headers, no size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, empty, 1), 0);
	/* No PT_LOAD headers, no size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, &empty[1], 1), 0);
	/* Empty PT_LOAD and non-PT_LOAD headers, no size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, empty, 2), 0);

	/* Normal set of PT_LOADS, and expected size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, mount, ARRAY_SIZE(mount)), mount_size);
	/* Unordered PT_LOADs result in same size. */
	KUNIT_EXPECT_EQ(test, total_mapping_size(NULL, unordered, ARRAY_SIZE(unordered)), mount_size);
}
