...
static inline pte_t pte_advance_phys(pte_t pte, unsigned long bytes)
{
	...
	/*
	 * pte_pgprot() preserves offsets within a native page for pfn_pte()
	 * round trips. Strip the full encoded address when advancing by bytes.
	 */
	pteval_t prot = pte_val(pte) ^ __phys_to_pte_val(phys);
...
static inline
pud_t *pud_offset_lockless_mm(struct mm_struct *mm, p4d_t *p4dp, p4d_t p4d,
			      unsigned long addr)
{
	if (!pgtable_l4_enabled())
		return p4d_to_folded_pud_mm(mm, p4dp, addr);
	return (pud_t *)__va(p4d_page_paddr(p4d)) + pud_index_mm(mm, addr);
}
...
static inline pud_t *pud_offset_mm(struct mm_struct *mm, p4d_t *p4dp,
				   unsigned long addr)
{
	return pud_offset_lockless_mm(mm, p4dp, READ_ONCE(*p4dp), addr);
}
...
static inline
p4d_t *p4d_offset_lockless_mm(struct mm_struct *mm, pgd_t *pgdp, pgd_t pgd,
			      unsigned long addr)
{
	if (!pgtable_l5_enabled())
		return pgd_to_folded_p4d_mm(mm, pgdp, addr);
	return (p4d_t *)__va(pgd_page_paddr(pgd)) + p4d_index_mm(mm, addr);
}
...
static inline p4d_t *p4d_offset_mm(struct mm_struct *mm, pgd_t *pgdp,
				   unsigned long addr)
{
	return p4d_offset_lockless_mm(mm, pgdp, READ_ONCE(*pgdp), addr);
}
