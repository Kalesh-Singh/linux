					 ...
					 unsigned long tg)
{
	...
	arg |= FIELD_PREP(TLBIR_BADDR_MASK, addr >> (lpa2 ? 16 : (tg == TLBI_TTL_TG_4K ? PAGE_SHIFT_4KB : PAGE_SHIFT)));
	arg |= FIELD_PREP(TLBIR_TTL_MASK, level > 3 ? 0 : level);
	arg |= FIELD_PREP(TLBIR_NUM_MASK, num);
	arg |= FIELD_PREP(TLBIR_SCALE_MASK, scale);
	arg |= FIELD_PREP(TLBIR_TG_MASK, tg);
						 ...
						 unsigned long tg)
{
	...
	while (addr != end) {
		...
		if (num >= 0) {
			__tlbi_range(rop, addr, asid, scale, num, level, lpa2, tg);
			addr += __TLBI_RANGE_PAGES(num, scale) * stride;
		}
...
#define __flush_s1_tlb_range_op(op, start, pages, stride, asid, tlb_level, tg) \
	__flush_tlb_range_op(op, r##op, start, pages, stride, asid, tlb_level, lpa2_is_enabled(), tg)

#define __flush_s2_tlb_range_op(op, start, pages, stride, tlb_level) \
	__flush_tlb_range_op(op, r##op, start, pages, stride, 0, tlb_level, kvm_lpa2_is_enabled(), get_trans_granule())
					...
					tlbf_t flags)
{
	...
	unsigned long tg = mm_get_trans_granule(mm);

	pages = (end - start) >> mm_pte_shift(mm);
				   ...
				   unsigned long start, unsigned long end)
{
	 ...
	 * information here.
	 */
	__flush_tlb_range(vma, start, end, mm_pte_size(vma->vm_mm), TLBI_TTL_UNKNOWN, TLBF_NONE);
}
...
static inline void flush_tlb_kernel_range(unsigned long start, unsigned long end)
{
	...
	__flush_s1_tlb_range_op(vaale1is, start, pages, stride, 0,
				TLBI_TTL_UNKNOWN, get_trans_granule());
	__tlbi_sync_s1ish_kernel();
	isb();
}
		...
		struct mm_struct *mm, unsigned long start, unsigned long end)
{
	...
	__flush_tlb_range(&vma, start, end, mm_pte_size(mm), 3,
