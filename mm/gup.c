...
int __mm_populate(unsigned long start, unsigned long len, int ignore_errors)
{
	...
	for (nstart = start; nstart < end; nstart = nend) {
		...
		nend = nstart + ((unsigned long)ret << MM_PAGE_SHIFT(mm));
		ret = 0;
	}
	if (locked)
		mmap_read_unlock(mm);
		...
		int *nr)
{
	...
	do {
		...
		pages[*nr] = page;
		(*nr)++;
	} while (ptep++, addr += MM_PAGE_SIZE(PGTABLE_MM()), addr != end);
		...
		unsigned int gup_flags, struct page **pages)
{
	...
	start = untagged_addr(start) & MM_PAGE_MASK();
	len = (unsigned long)nr_pages << MM_PAGE_SHIFT();
	...
	start += (unsigned long)nr_pinned << MM_PAGE_SHIFT();
	pages += nr_pinned;
