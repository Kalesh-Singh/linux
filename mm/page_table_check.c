				  ...
				  pmd_t pmd)
{
	...
	if (pmd_user_accessible_page(mm, addr, pmd))
		page_table_check_clear(pmd_pfn(pmd), MM_PMD_SIZE(mm) >> PAGE_SHIFT);
}
				  ...
				  pud_t pud)
{
	...
	if (pud_user_accessible_page(mm, addr, pud))
		page_table_check_clear(pud_pfn(pud), MM_PUD_SIZE(mm) >> PAGE_SHIFT);
}
...
{
	unsigned int i;
	unsigned long pgsize = mm_pte_size(mm);
	...
	for (i = 0; i < nr; i++) {
		unsigned long cur_addr = addr + pgsize * i;
		...
		if (pte_user_accessible_page(mm, cur_addr, new_pte))
			page_table_check_set(pte_pfn(new_pte), nr_pages, pte_write(new_pte));
	}
}
		...
		pmd_t *pmdp, pmd_t pmd, unsigned int nr)
{
	...
	for (i = 0; i < nr; i++)
		__page_table_check_pmd_clear(mm, addr + MM_PMD_SIZE(mm) * i, *(pmdp + i));
	if (pmd_user_accessible_page(mm, addr, pmd))
		...
		pud_t *pudp, pud_t pud,	unsigned int nr)
{
	...
	for (i = 0; i < nr; i++)
		__page_table_check_pud_clear(mm, addr + MM_PUD_SIZE(mm) * i, *(pudp + i));
	if (pud_user_accessible_page(mm, addr, pud))
