...
#define VMG_MMAP_STATE(name, map_, vma_)				\
	struct vma_merge_struct name = {				\
		...
		.pgoff = mmap_pgoff_offset(map_),			\
		.slice_off = mmap_slice_offset(map_),			\
		.file = (map_)->file,					\
		.prev = (map_)->prev,					\
		.middle = vma_,						\
...
static void vmg_adjust_set_range(struct vma_merge_struct *vmg)
{
	...
	/*
	 * We calculate the new offsets at vmg->end using middle as the
	 * reference VMA because vmg->end is guaranteed to fall within the
	 * range of middle. Since middle and next are contiguous in the file,
	 * this is mathematically identical to shifting next's offset backward,
	 * but avoids unsigned underflow inside vma_pgoff_offset() /
	 * vma_slice_offset() since vmg->end < next->vm_start.
	 */
		...
		struct vma_merge_struct *vmg)
{
	...
	if (merge_both) {
		...
		if (vmg->__remove_middle) {
			...
			vmg->slice_off = vma_slice_off(middle);
		} else {
		  ...
		  bool unlock)
{
	...
	if ((mm_offset_in_pte(mm, start)) || start > TASK_SIZE || len > TASK_SIZE-start)
		return -EINVAL;

	end = start + mm_pte_align(mm, len);
	if (end == start)
		return -EINVAL;
	...
	struct mmap_action *action)
{
	...
	vma_set_range(vma, map->addr, map->end, mmap_pgoff_offset(map));
	vma_set_slice_off(vma, mmap_slice_offset(map));
...
int expand_upwards(struct vm_area_struct *vma, unsigned long address)
{
	...
	address &= mm_pte_mask(mm);
	if (address >= (TASK_SIZE & mm_pte_mask(mm)))
		return -ENOMEM;
	address += mm_pte_size(mm);
	...
	if (address > vma->vm_end) {
		...
		grow = (address - vma->vm_end) >> mm_pte_shift(mm);

		error = -ENOMEM;
		if (vma->vm_pgoff + (size >> mm_pte_shift(mm)) >= vma->vm_pgoff) {
...
int expand_downwards(struct vm_area_struct *vma, unsigned long address)
{
	...
	if (address < vma->vm_start) {
		...
		grow = (vma->vm_start - address) >> mm_pte_shift(mm);
